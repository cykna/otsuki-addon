...
export var CompressionMethod = /*#__PURE__*/ function(CompressionMethod) {
    CompressionMethod["Lz"] = "L";
    CompressionMethod["CborPako"] = "C";
    return CompressionMethod;
}({});
export function compress(data) {
    const value = data.length < 512 ? `${"L"}${lzcompress(data)}` : `${"C"}${compress_cbor_pako(data)}`;
    return value;
}
export function decompress(data) {
    const last = data[0];
    if (last == "L") return lzdecompress(data.slice(1));
...
/**
  ...
  */ export function compress_cbor_pako(data) {
...
*/ export function decompress_cbor_pako(data) {
    const out = [];
